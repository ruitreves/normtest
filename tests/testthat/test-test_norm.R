...
test_that("shaprio works", {
  expect_true(setequal(pass2, unlist(res[[1]])))
})
test_that("levene works", {
  expect_true(setequal(setdiff(pass1, pass2), unlist(res[[2]])))
})
test_that("both work", {
  expect_true(setequal(setdiff(x, union(pass1, pass2)), unlist(res[[3]])))
})
