...
and "KW_results.csv". It will also assign three variables tier1, tier2, and tier3 to your global r environment. 
If you have important variables with these names, please note that this function will overwrite them. 

Also note that depending on the size of your data, this function could take several minutes to run. 

From here, the rest is up to you, but we will demonstrate what next steps are useful for our purposes.
...
Then, maybe we want to see which fold changes are the largest (considering absoulute values). 

```{r eval = TRUE}
ordered_comp <- comp[order(abs(comp[, 2]), decreasing = TRUE), ]
head(ordered_comp)
```
...
```{r eval = TRUE}
ordered_comp <- comp[is.finite(comp[, 2]), ]
ordered_comp <- ordered_comp[order(abs(ordered_comp[, 2]), decreasing = TRUE), ]
head(ordered_comp)
```

Now these values are more useful to us. 

Both of these use cases are only that, specific use cases. There is much more you can do 
with this data after processing, and this is likely dependent on what kind of data you have
and of course, what you're trying to achieve. 
